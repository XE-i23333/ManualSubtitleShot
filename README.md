...
手动截取视频字幕并拼接成长图的轻量化Windows工具。按F9截图，去重边缘并拼接，实时预览，最终导出 PNG 长图。

## 功能

- **F8** 框选视频区域（支持跨屏）
- **F9** 截图
- **实时预览**：每次截图后增量更新拼图预览（为提高预览性能与减小拼接缓存内存占用，预览上限 960px 宽，放大查看时略糊属正常）；**导出为高清原图**
- **截图管理**：缩略图列表，可单独删除任意一帧；第一张为背景图
- **保存 PNG**：拼接导出
- **主题跟随**：自动检测 Windows 深浅色模式（`AppsUseLightTheme`）并切换 UI 主题
- **窗口置顶**：标题栏内置置顶按钮
...
本项目依赖 [果核剥壳core-ui](https://github.com/ghboke/core-ui/)。目录布局：

```
工作区/
└── manualsubtitleshot_coreui/
    ├── core-ui-v1.7.0/      # core-ui 库（include + lib/static + lib/dynamic，见下）
    └── manualsubtitleshot_coreui.slnx
```

Debug（vs设置为动态链接）与 Release（vs设置为静态链接）需要 core-ui 两套产物。（故意这么设置的，都设为static或dynamic也可以）

### 第一步：获取 core-ui

**方式 A（不推荐）：下载预编译 release**

从 [果核core-ui Releases](https://github.com/ghboke/core-ui/releases) 下载 `core-ui-v1.7.0-windows-x64.zip`，解压后把 `core-ui-v1.7.0\` 目录放到本仓库根（与 `manualsubtitleshot_coreui.slnx` 同级）即可。包内已含 dynamic（`core-ui.dll` + 导入库）与 static（含 QuickJS / LunaSVG 的 `core-ui.lib`）两套产物。

> **注意（为什么 Release 必须自编 static 库）**：本项目的目标之一就是摆脱一切Runtime独立运行，官方预编译包里的 static `core-ui.lib` 是按 **/MD（动态 CRT）** 编译的——core-ui 代码虽然静态进了 exe，但 MSVC 运行库（vcruntime140.dll / msvcp140.dll 等）仍需目标机器安装 **VC++ Redistributable**，否则启动报"缺少 VCRUNTIME140.dll"。而且它与本仓库 Release 的 /MT 配置不匹配，会直接链接失败（LNK2038 `RuntimeLibrary` 不匹配）。
...
Dynamic core-ui（Debug用）—— 产出 `core-ui.dll` + 导入库，install 前缀设为项目根，自动生成 `core-ui-v1.7.0\`：
...
Copy-Item "$COREUI\build-fat\core-ui-fat.lib" "<项目根>\core-ui-v1.7.0\lib\static\core-ui.lib" -Force
```

最终 `manualsubtitleshot_coreui\core-ui-v1.7.0\` 内容：
...
```powershell
# Debug（动态链接 core-ui.dll，自动拷贝 core-ui-v1.7.0\lib\dynamic）
